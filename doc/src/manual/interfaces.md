...
| `IndexMethod(::Type)`                           | `IndexCartesian()`                       | Returns either `IndexLinear()` or `IndexCartesian()`. See the description below.      |
...
A key part in defining an `AbstractArray` subtype is [`IndexMethod`](@ref). Since indexing is
...
julia> Base.IndexMethod{T<:SquaresVector}(::Type{T}) = IndexLinear()
...
Notice that this is a `IndexCartesian` array, so we must manually define [`getindex()`](@ref) and [`setindex!()`](@ref)
