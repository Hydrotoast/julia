...
module IteratorsMD
    import Base: eltype, length, size, start, done, next, last, in, getindex,
                 setindex!, IndexMethod, min, max, zero, one, isless, eachindex,
...
@inline to_indices{N}(A, inds, I::Tuple{Union{Array{Bool,N}, BitArray{N}}}) =
    (_maybe_linear_logical_index(IndexMethod(A), A, I[1]),)
_maybe_linear_logical_index(::IndexMethod, A, i) = to_index(A, i)
...
## setindex! ##
@generated function _setindex!(l::IndexMethod, A::AbstractArray, x, I::Union{Real, AbstractArray}...)
    N = length(I)
    quote
        @_inline_meta
        @boundscheck checkbounds(A, I...)
...
@generated function _unsafe_setindex!(::IndexMethod, A::AbstractArray, x, I::Union{Real,AbstractArray}...)
    N = length(I)
    quote
        X = _iterable(x)
        @nexprs $N d->(I_d = I[d])
...
function copy!{T,N}(dest::AbstractArray{T,N}, src::AbstractArray{T,N})
    ...
    for I in eachindex(IndexMethod(src,dest), src)
