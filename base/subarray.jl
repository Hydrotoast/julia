...
function SubArray(parent::AbstractArray, indexes::Tuple)
    @_inline_meta
    SubArray(IndexMethod(viewindexing(indexes), IndexMethod(parent)), parent, ensure_indexable(indexes), index_dimsum(indexes...))
end
...
IndexMethod(::Type{<:FastSubArray}) = IndexLinear()
IndexMethod(::Type{<:SubArray}) = IndexCartesian()
