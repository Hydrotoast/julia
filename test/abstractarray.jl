...
Base.IndexMethod{A<:T24Linear}(::Type{A}) = IndexLinear()
...
Base.IndexMethod{A<:TSlow}(::Type{A}) = IndexCartesian()
...
Base.IndexMethod(::UnimplementedFastArray) = Base.IndexLinear()

mutable struct UnimplementedSlowArray{T, N} <: AbstractArray{T, N} end
Base.IndexMethod(::UnimplementedSlowArray) = Base.IndexCartesian()
...
Base.IndexMethod{A<:TSlowNIndexes}(::Type{A}) = Base.IndexCartesian()
